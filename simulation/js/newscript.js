...
function varupdate() {
  ...
  printcomment(
    "Simple Harmonic Motion <br> a = - &omega;<sup>2</sup> * A * cos( &omega; t )",
    1
  );
  ...
  if (simstatus) {
    ...
    printcomment(
      "Acceleration (mm/s<sup>2</sup>) <br> a = -" +
        omega +
        "<sup>2</sup> &times; " +
        l +
        " &times; cos(" +
        theta +
        "&deg;)</br>= " +
        roundd(-omega * omega * l * Math.cos(rad(theta)), 2),
      2
    );
  }
...
function draw() {
  ...
  ctx.fillText("Acceleration Analysis", 15, 15);
  ctx.restore();
  //ptx.push(p.xcoord);
  //pty.push(p.ycoord);
  disprem(ctx);
...
function graph(context) {
  if (document.getElementById("graphPlot").checked == true) {
    if (!simstatus) {
      ...
      ptxddot.push(o.xcoord + 145 - l * omega * omega * Math.cos(rad(theta)));
      pty.push(o.ycoord - j);
      j = j + 1;
    }
    ...
    plotgraph(ptxddot, pty, context, truncate, "#FF0000");
  } else {
    ptx = [];
    ptxdot = [];
    ptxddot = [];
